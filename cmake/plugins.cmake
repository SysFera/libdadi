# TODO: comment
macro(add_plugin _NAME)
  add_library(${_NAME} MODULE ${ARGN})
  target_link_libraries(${_NAME} ${LIBS})
  message(STATUS "name: ${_NAME}")
  set(PLUGIN_NAME ${_NAME})
  set(PLUGIN_PATH ${LIBRARY_OUTPUT_PATH})
  set_target_properties(${PLUGIN_NAME} PROPERTIES DEFINE_SYMBOL "DIET_EXPORTS")
  configure_file(${CMAKE_CURRENT_SOURCE_DIR}/${PLUGIN_NAME}.xml.in ${MANIFEST_OUTPUT_PATH}/${PLUGIN_NAME}.xml)
endmacro()


macro(install_plugin _NAME _PATH)
  install(TARGETS ${_NAME}
    COMPONENT runtime
    ARCHIVE DESTINATION ${PLUGINS_INSTALL_DIR}/${_PATH}
    LIBRARY DESTINATION ${PLUGINS_INSTALL_DIR}/${_PATH}
    RUNTIME DESTINATION ${PLUGINS_INSTALL_DIR}/${_PATH})
  set(PLUGIN_NAME ${_NAME})
  set(PLUGIN_PATH ${CMAKE_INSTALL_PREFIX}/${PLUGINS_INSTALL_DIR}/${_PATH})
  configure_file(${CMAKE_CURRENT_SOURCE_DIR}/${PLUGIN_NAME}.xml.in ${CMAKE_CURRENT_BINARY_DIR}/${PLUGIN_NAME}.xml)
  install(FILES ${CMAKE_CURRENT_BINARY_DIR}/${PLUGIN_NAME}.xml
    DESTINATION ${PLUGINS_INSTALL_DIR}/${_PATH})
endmacro()