...
Il important de noter que la politique de sécurité présentée dans ce document devra être la plus générique possible pour pouvoir
s'adapter aux différentes infrastructures dans lesquelles le \textit{middleware DIET} sera déployé.

\section{ Proposition n\textdegree{}1: Utilisation de UMS }
     
\subsection{ Authenfication des utilisateurs }
...
\section{ Proposition n\textdegree{}2: Gestion des certificats }
...
\subsection{ Les infrastructures à clés publiques (PKI en anglais) }
...
\subsubsection[{ Les fonctions d'une architecture PKI [Securix] }]{ Les fonctions d'une architecture PKI \hyperlink{Securix}{[Securix]} }
...
\subsection{ Une infrastructures à clé publique pour DIET V3 }
...
\item Le CA contrôle le certificat en vérifiant la correspondance entre clé publique et identité de l'utilisateur.
\item Le CA confirme ou refute le certificat.
\item Le CA déchiffre le message à l'aide de sa clé privée et le chiffre à l'aide de la clé publique de l'élément de la
hiérarchie DIET correspondant.
\item Si le certificat est valide, l'élement de la hiérarchie DIET correspondant déchiffre le message via sa clé 
privée et chiffre sa réponse via la même clé publique de l'utilisateur.
\end{itemize}
